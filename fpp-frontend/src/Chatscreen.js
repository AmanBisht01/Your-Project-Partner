  ...
  useEffect(() => {
    if (roomId) {
      ...
      const msg = messages.filter((val) => {
        return (
          (val.sender === logId || val.sender === roomId) &&
          (val.receiver === roomId || val.receiver === logId)
        );
      });
      console.log(msg);
      setUsermsg(msg);
  ...
  return (
    <div className="chatscreen">
      <div className="chatscreen_header">
        <Avatar
          alt={receiver.name}
          src={`${process.env.PUBLIC_URL}/uploads/${receiver.profile}`}
        />
      ...
      <div className="chatscreen_div">
        {usermsg.map((message) => (
          <p
            className={`chatscreen_text ${
              message.sender === logId && "chatscreen_user"
            }`}
          >
            {message.message}
          </p>
